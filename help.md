...
| Command | Description | Example |
| --- | --- | --- |
| `.model [name or reset]` | No args: show current and server-fetched models. With `name`: change model. Use `reset` to restore default. | `.model gpt-5-mini` |
| `.tools [on|off|toggle|status]` | Toggle hosted OpenAI tools and MCP access on or off. | `.tools toggle` |
