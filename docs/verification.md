...
- Device state (Olm/Megolm keys, trust) is persisted in the `store/` directory and tied to the `device_id`.
...
- `MATRIX_E2E`: enable/disable encryption. When disabled, verification is not used.
- `MATRIX_STORE_PATH`: where encryption state is stored (default: `store/`). Back it up cautiously; it contains sensitive keys.
- `MATRIX_DEVICE_ID`: optional persisted device ID. Changing it creates a new device from Matrix’s perspective.
