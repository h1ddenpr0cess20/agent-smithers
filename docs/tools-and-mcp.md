...
- Local media function tools are broader than hosted tools. When `XAI_API_KEY` is configured, this project exposes Grok Imagine image tools and a `generate_video` tool to both OpenAI and xAI chat models.
...
Video generation through Grok costs real money per call. To prevent surprise bills, you can restrict who is allowed to generate video using the `VIDEO_WHITELIST` environment variable or the `.whitelist` admin command.
